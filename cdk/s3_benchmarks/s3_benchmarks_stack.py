...
DEFAULT_S3_CLIENTS = {
    ...
    'sdk-java-client-classic': S3ClientProps(color="#4b3619"),  # COFFEE
    'sdk-java-tm-classic': S3ClientProps(color="#4b382a"),  # ESPRESSO
...
class S3BenchmarksStack(Stack):
    ...
    def _define_all_per_instance_batch_jobs(self):
        ...
        commands_user_data.add_commands('mkdir /uniquework')
        commands_user_data.add_commands('mount /dev/nvme1n1 /uniquework')
    ...
    def _define_per_instance_batch_job(self, instance_type: s3_benchmarks.InstanceType):
        ...
        container_defn = batch.EcsEc2ContainerDefinition(
            ...
            volumes=[batch.EcsVolume.host(container_path=s3_benchmarks.S3_BENCHMARKS_WORK_BASE_DIR, host_path=s3_benchmarks.S3_BENCHMARKS_WORK_BASE_DIR,
                                          name="uniquework")] if instance_type.storage_configuration == s3_benchmarks.StorageConfiguration.INSTANCE_STORAGE else None,
        )
