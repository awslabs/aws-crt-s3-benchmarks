...
csv_path = sys.argv[1]
output = sys.argv[3] if len(sys.argv) > 3 and sys.argv[2] == '--output' else csv_path.replace('.csv', '.png')
...
# Disk
axes[1].plot(df['elapsed'], df['disk_write_mbs'] / 1024, label='Write', color='red')
axes[1].plot(df['elapsed'], df['disk_read_mbs'] / 1024, label='Read', color='green')
...
axes[3].axhline(y=df['mem_total_gib'].iloc[0], color='gray', linestyle='--', label='Total')
