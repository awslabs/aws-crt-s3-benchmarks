...
public class SDKJavaTaskTransferManager implements SDKJavaTask  {
        ...
        if(runner.transferKey != null) {
            ...
            if(runner.transferAction.equals("upload")) {
                DirectoryUpload directoryUpload =
                        runner.transferManager.uploadDirectory(UploadDirectoryRequest.builder()
                                .source(runner.transferPath)
                ...
                DirectoryDownload directoryUpload =
                        runner.transferManager.downloadDirectory(DownloadDirectoryRequest.builder()
                                .destination(runner.transferPath)
                                .bucket(runner.bucket)
    ...
    private void existWithErrorHelper(SDKJavaBenchmarkRunner runner) {
        Util.exitWithError(String.format("Transfer with action:%s to path:%s failed", runner.transferAction, runner.transferPath));
    }
}
